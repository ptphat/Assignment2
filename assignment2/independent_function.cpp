...
std::string independent_function::getid_book(int size)
{
	string ID, s = currentDateTime();
	ID = ID + s[2] + s[3];
	if (size >= 1000){
...
std::string independent_function::getpass()
{
	char ch;
	string pass;
	int count = 0;
...
int independent_function::clean_stdin()
{
	while (getchar() != '\n') {
	}
	return 1;
...
int independent_function::getchoice(int begin, int end)
{
	int choice;
	char c;
	do{
...
const string independent_function::currentDateTime()
{
	time_t     now = time(0);
	struct tm  tstruct;
	char       buf[80];
...
void independent_function::welcome()
{
	system("cls");
	system("color 0C");
	cout << endl << endl << endl;
...
void independent_function::loading()
{
	system("cls");
	cout << endl << endl << endl;
...
void independent_function::goodbye()
{
	system("cls");
	system("color 0C");
	cout << endl << endl << endl;
